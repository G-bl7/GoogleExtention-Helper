    ...
    request.onsuccess = function (event) {
        ...
        if (textNote) {
            ...
            updateRequest.onerror = function (event) {
                console.error(`Error updating ${field} for text note ID ${textNoteID}:`, event.target.error);
            };
        } else {
            ...
            funcGetDefaultProfile(db, (defaultProfile) => {
                chrome.scripting.executeScript({
                    ...
                    func: (text, defaultProfile) => {
                        ...
                        if (note !== null) {  // Check if note is not null (user didn't cancel)
                            let textNoteObj = {
                                text: text,
                                note: note,
                            ...
                            chrome.runtime.sendMessage({ action: 'addNewTextNote', textNote: textNoteObj }, (response) => {
                            });
